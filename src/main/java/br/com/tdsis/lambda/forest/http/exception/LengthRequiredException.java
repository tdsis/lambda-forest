package br.com.tdsis.lambda.forest.http.exception;

import org.apache.http.HttpStatus;

/**
 * The LengthRequiredException class
 * <p>
 * This is a concrete class of the the HttpException class. 
 * It represents a length required http response.
 * 
 * @author fsantana
 * @version 0.0.2
 * @since 0.0.2
 * @see {@link HttpStatus#SC_LENGTH_REQUIRED}
 */
public class LengthRequiredException extends HttpException {

    private static final long serialVersionUID = 3099405537692347007L;
    private static final int HTTP_STATUS = HttpStatus.SC_LENGTH_REQUIRED;

    public LengthRequiredException() {
        super(HTTP_STATUS);
    }

    public LengthRequiredException(Object entity) {
        super(entity, HTTP_STATUS);
    }

    public LengthRequiredException(final String message, final Throwable cause) {
        super(HTTP_STATUS, message, cause);
    }

    public LengthRequiredException(Object entity, final String message, final Throwable cause) {
        super(entity, HTTP_STATUS, message, cause);
    }

}
